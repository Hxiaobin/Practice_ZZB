...
import static com.sf.bocfinancialsoftware.constant.URLConfig.FORWARD_MESSAGE_LIST_URL;
...
public class ForwardMessageListActivity extends BaseActivity implements View.OnClickListener, SwipeRefreshLayout.OnRefreshListener, AbsListView.OnScrollListener {
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.lltFilterCondition1:  //筛选远期外汇
                ...
                requestByQueryFilter(filter);
                break;
            case R.id.lltFilterCondition2: //筛选外汇资金业务通知
                filter = QUERY_FORWARD_CONDITION2;
                requestByQueryFilter(filter);
                break;
            case R.id.lltFilterCondition3: //筛选代理结售汇业务提示
                filter = QUERY_FORWARD_CONDITION3;
                requestByQueryFilter(filter);
                break;
            case R.id.lltFilterCondition4: //进口代客外汇理财业务提示
                filter = QUERY_FORWARD_CONDITION4;
                requestByQueryFilter(filter);
                break;
            case R.id.lltFilterCondition5: //进口代客外汇风险管理提示
    ...
    public void onRefresh() {
        ...
        if (map.get(QUERY_FILTER) == null) { //没有筛选条件
            ...
            map.put(QUERY_PAGE, String.valueOf(page)); //查询页码
            getNetworkData(FORWARD_MESSAGE_LIST_URL, map, strSuccess, strError, REQUEST_FROM_REFRESH);
        }
    }
        ...
        if (scrollState == SCROLL_STATE_IDLE && isLastLine) { //停止滚动，且滚动到最后一行
            if (hasNext.equals(HAS_NOT_NEXT)) { // 如果没有下一页
                ...
                if (map.get(QUERY_FILTER) == null) { //没有筛选条件
                    ...
                    map.put(QUERY_FILTER, filter);
                    map.put(QUERY_PAGE, String.valueOf(page));
                    getNetworkData(FORWARD_MESSAGE_LIST_URL, map, strSuccess, strError, REQUEST_FROM_LOAD_MORE);
                }
            }
        }
    }
    ...
    private void firstRequest() {
        ...
        messageAdapter = new MessageAdapter(ForwardMessageListActivity.this, msgArray);
    ...
    public void requestByQueryFilter(String strFilter) {
        ...
        getNetworkData(FORWARD_MESSAGE_LIST_URL, map, strSuccess, strError, REQUEST_FROM_FILTER);
        mPopWindow.dismiss();
    }
    ...
    public void getNetworkData(String url, final HashMap<String, String> mapObject, final String success, final String error, final String flag) {
        HttpUtil.getNetworksJSonResponse(ForwardMessageListActivity.this, url, mapObject, new HttpCallBackListener() {
            @Override
            public void onSuccess(String response) {
                ...
                msgArray.addAll(bean.getContent().getMsgArray());  //添加中银分析数据到List中
                ...
                Toast.makeText(ForwardMessageListActivity.this, success, Toast.LENGTH_SHORT).show();
            }
            ...
            public void onFailed(String msg) {
                ...
                lltLoadMore.setVisibility(View.GONE); //隐藏正在加载
                Toast.makeText(ForwardMessageListActivity.this, error, Toast.LENGTH_SHORT).show();
            }
            ...
            public void onError(Exception e) {
                ...
                lltLoadMore.setVisibility(View.GONE);  //隐藏正在加载
                Toast.makeText(ForwardMessageListActivity.this, e.getMessage(), Toast.LENGTH_SHORT).show();
            }
        });
    }
